...
//		stroke.isOnKeyRelease()
    ...
    /**
	 * 从给定的偏移位置向前搜索，如果碰到空格、回车、换行、tab等符号时返回当前的位置
	 * 
	 * @param offset
	 *            --给定的偏移量
	 * @return --向前搜索，遇到的第一个whitespace符号的位置
	 */
    ...
    public int getWordEnd(int offset)
    {
        ...
        offset++;
        str = getPositionChar(offset);//取后一个字符
        if(StringUtil.isSpaceString(str)||str.equals(","))
        {
            return offset-1;
        }else
    ...
    /**
     * 与方法getBeforeBlankString类似，增加了“,”作为分隔符
     * @param offset
     * @return
     */
