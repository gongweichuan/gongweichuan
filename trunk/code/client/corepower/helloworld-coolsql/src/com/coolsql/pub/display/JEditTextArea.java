...
import com.coolsql.pub.util.TextUtilities;
import com.coolsql.system.Setting;

/**
 * This editor is an extension of JEditTextArea.
...
public class JEditTextArea extends JComponent
	...
	private static final Logger logger = Logger.getLogger(JEditTextArea.class);
	public static final ActionMap actions = new ActionMap();
	static {
		actions.put("copy", new EditorAction("copy"));
		actions.put("cut", new EditorAction("cut"));
		actions.put("paste", new EditorAction("paste"));
		actions.put("selectall", new EditorAction("selectall"));
	}
	...
	private void initHotKey() {
		addKeyBinding("C+X", actions.get("cut"));
		addKeyBinding("C+C", actions.get("copy"));
		addKeyBinding("C+V", actions.get("paste"));
		addKeyBinding("C+A", actions.get("selectall"));
	}
	...
	public void addKeyBinding(KeyStroke aBinding, ActionListener aListener) {
		this.getInputHandler().addKeyBinding(aBinding, aListener);
	}
...
//	@SuppressWarnings("deprecation")
	...
	private void startBlinkTimer() {
		...
		caretTimer = new Timer(blinkInterval, new ActionListener() {
	...
	public String getText() {
		...
		try {
			...
			logger.error("Error setting text", bl);
			return null;
		}
	}
	...
	public void setTabSize(int aSize) {
		document
				.putProperty(PlainDocument.tabSizeAttribute, new Integer(aSize));
	}
	...
	public void appendLine(String aLine) {
		try {
			...
			document.insertString(document.getLength(), fixLinefeed(aLine),
					null);
		} catch (BadLocationException bl) {
			logger.error("Error setting text", bl);
		} finally {
	...
	/**
	 * Returns the word that is left of the cursor. If the character left of the
	 * cursor is a whitespace this method returns null.
	 * 
	 * @param wordBoundaries
	 *            additional word boundary characters (whitespace is always a
	 *            word boundary)
	 */
	...
	public void selectWordAtCursor(String wordBoundaries) {
		...
		if (pos <= 0)
			return;
		if (Character.isWhitespace(line.charAt(pos - 1)))
			return;
	...
	public void processKeyEvent(KeyEvent evt) {
		...
		if(evt.isConsumed())
			return;
		int oldcount = NumberStringCache.getNumberString(this.getLineCount())
				.length();
		switch (evt.getID()) {
			case KeyEvent.KEY_TYPED :
		...
		int newcount = NumberStringCache.getNumberString(this.getLineCount())
				.length();
		boolean changed = false;
	...
	protected void fireTextStatusChanged(boolean isModified) {
		...
		for (int i = list.length - 2; i >= 0; i--) {
			if (list[i] == TextChangeListener.class) {
				((TextChangeListener) list[i + 1])
						.textStatusChanged(isModified);
			}
		}
	}
	...
	protected void fireSelectionEvent() {
		...
		for (int i = list.length - 2; i >= 0; i--) {
			if (list[i] == TextSelectionListener.class) {
				((TextSelectionListener) list[i + 1]).selectionChanged(this
						.getSelectionStart(), this.getSelectionEnd());
			}
		}
	}
	...
	// this.statusBar.setEditorLocation(line + 1,
	// this.getCaretPositionInLine(line) + 1);
	// }
	// }
	...
	protected void documentChanged(DocumentEvent evt) {
		...
		if (count == 0) {
			...
			painter.invalidateLineRange(line, (firstLine < 0 ? 0 : firstLine)
					+ visibleLines);
		}
			...
			EventQueue.invokeLater(new Runnable() {
				public void run() {
					if (evt.getAdjustable() == vertical)
						...
						setHorizontalOffset(horizontal != null ? -horizontal
								.getValue() : 0);
				}
			});
		}
	...
	class DragHandler implements MouseMotionListener {
		public void mouseDragged(MouseEvent evt) {
			if (popup != null && popup.isVisible())
				return;
			if (popup != null&&(evt.getModifiers() & InputEvent.BUTTON3_MASK) != 0) {
				return;
			}
			setSelectionRectangular((evt.getModifiers() & Setting.getInstance()
					.getRectSelectionModifier()) != 0);
	...
	class MouseHandler extends MouseAdapter {
		...
		public void mousePressed(MouseEvent evt) {
			...
			isPopupTriggerWhenPress=evt.isPopupTrigger();
		...
		public void mouseReleased(MouseEvent evt)
		{
			...
			int offset = xToOffset(line, x);
			int dot = getLineStartOffset(line) + offset;
			if((isPopupTriggerWhenPress||evt.isPopupTrigger())&&popup != null){
//			if ((evt.getModifiers() & InputEvent.BUTTON3_MASK) != 0
//					&& popup != null) {
		...
		protected void doSingleClick(MouseEvent evt, int line, int offset,
				int dot) {
			if ((evt.getModifiers() & InputEvent.SHIFT_MASK) != 0) {
				rectSelect = (evt.getModifiers() & Setting.getInstance()
						.getRectSelectionModifier()) != 0;
		...
		protected void doDoubleClick(MouseEvent evt, int line, int offset,
			...
			String noWordSep = Setting.getInstance().getEditorNoWordSep();
			if (noWordSep == null)
				noWordSep = "";
		...
		protected void doTripleClick(MouseEvent evt, int line, int offset,
				int dot) {
	...
	/**
	 * 编辑器的编辑快捷动作处理类（copy,cut,paste,selectall）
	 * 
	 * @author kenny liu
	 * 
	 */
	private static class EditorAction extends AbstractAction {
		...
		private String name;
		/**
		 * 当前Action所依附的编辑组件。可以不设置
		 */
		private JEditTextArea edit;
		public EditorAction(String name) {
			this.name = name;
		}
		public void actionPerformed(ActionEvent e) {

			JEditTextArea editor = null;
			if (edit != null)
				editor = edit;
			else {
				Object ob = e.getSource();
				if (ob instanceof JEditTextArea) {
					editor = (JEditTextArea) ob;
				}
			}
			if ("copy".equals(name))
				editor.copy();
		 ...
		 *            the edit to set
		 */
		public void setEdit(JEditTextArea edit) {
			this.edit = edit;
		}
	}
}