 ...
 * 2008-1-6 create
 */
public class LogViewDisplayCheckListener implements IMenuLoadListener {
	...
	public void action(JMenuItem item) {
		if (item instanceof JCheckBoxMenuItem) {
			boolean isDisplay = Setting.getInstance().getBoolProperty(
					PropertyConstant.PROPERTY_VIEW_LOG_ISDISPLAY, true);
			if(!isDisplay)
				ViewManage.getInstance().getLogView().hidePanel(false);
			final JCheckBoxMenuItem m=(JCheckBoxMenuItem)item;
			m.setSelected(isDisplay);
			
			ViewManage.getInstance().getLogView().addPropertyChangeListener(new PropertyChangeListener()
			{
