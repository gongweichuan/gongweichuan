...
import com.coolsql.sql.dataview.cellcomponent.BlobDescriptor;
...
//					    }
	...
	private Object getDefaultValue(ResultSet set,int index, int maxColumnWidth, String encoding) throws SQLException
	{
        ...
        try {
            if (columnType == TypesHelper.LONGVARBINARY) //该类型的字段数据，使用二进制流来获取数据
                value = getEncodedStringFromBinaryStream(set,
                		encoding, index,maxColumnWidth);
            else if (rsmd.getColumnDisplaySize(index)< maxColumnWidth)  //使用普通字节(byte)字段来获取数据
                value = getEncodedString(set, encoding, index);
            else if ("".equals(encoding))           //如果字段长度超过了显示长度，并且编码格式没有指定，采用字符流来获取数据       
                value = getStringFromCharacterSteam(set, index,maxColumnWidth);
            else                                        //其他情况都采用二进制流来获取数据
                value = getEncodedStringFromBinaryStream(set,
                		encoding, index,maxColumnWidth);
        } catch (IOException e) {
