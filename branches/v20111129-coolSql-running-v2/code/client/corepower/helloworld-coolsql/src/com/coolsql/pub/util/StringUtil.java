...
import com.coolsql.pub.display.NumberStringCache;
...
public class StringUtil
{
   ...
   * @param checkBrackets flag to check for opening and closing brackets (delimiter inside brackets will not be taken into account)
    ...
    /**
     * 将日期对象转换成yyyy-MM-dd HH:mm:ss格式的字符串
     * @param date
     * @return
     */
     ...
     * 判断给定的字符串,是否由数字组成
     * @param str  --指定的字符串
     * @return  --如果由数字组成,返回true  否则返回false
     */
    ...
    /**
     * 根据给定日期，然后向前或者向后追溯days天
     * @param date  --被追溯的日期  如果为负数，那么向前追溯，如果为正数向后追溯
     * @param days  --追溯的天数
     * @return   --追溯后的日期（String）
     */
     ...
     * 根据给定的sql字段类型，获取时间格式
     * @param type  --sql字段类型
     * @return  --日期时间格式,其定义与IDateSelector相同
     */
     ...
     * 获取指定对象的hash名字。
     * @param ob  
     * @return  --返回由hash码和对象所对应的类名组成的字符串
     */
    ...
    /**
     * 对给定字符串进行是否为数值的校验，如果不是数值型，返回不合法的提示信息
     * @param str  --需要校验的字符串对象
     * @param size  --数值的最大长度
     * @param scal  --数值小数部分长度
     * @return  --如果为数值型，返回null  ；如果不是数值型，返回相应的错误信息
     */
    public static String isNumberString(String str,int size,int scale)
    {
        ...
        int decPos = str.indexOf(".");  //小数点部分
        
        String strIntPart="";//整数部分
        String strDecPart=""; //小数部分
        if (decPos > -1) {
    ...
    /**
     * 该方法与isNumberString(String str,int size,int scale)都采用相同的校验规则，只不过，该方法不会返回相应的提示信息，处理速度上较快。
     * @param str  --需要校验的字符串对象
     * @param size  --数值的最大长度
     * @param scale  --数值小数部分长度
     * @return  --如果符合数值型规则，返回true，否则返回false
     */
    ...
    /**
     * 校验字符串长度是否在给定的范围内，该校验分为按字节和按字符两种方式进行长度计算。
     * @param str  --需要校验的字符串
     * @param len  --字符串长度
     * @param isByte  --true：按字节计算字符串长度 false：按字符计算长度
     * @return  true：在给定长度范围内，false:不在给定长度范围内
     */
